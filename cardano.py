...
def restore_special_chars(text):
    ...
    for word, symbol in pairs:
        ...
        result = result.replace(word.upper(), symbol)
    return result
...
def rotate_key(key, size):
    # [КРИПТО] Поворот координат отверстий на 90 градусов по часовой стрелке.
    # Новая координата (j, size-1-i) получается из старой (i, j) при вращении решётки.
    return [(j, size - 1 - i) for i, j in key]

def insert_chars(grid, key, text_iter):
    # [КРИПТО] Расстановка букв открытого текста в ячейки сетки, соответствующие текущим отверстиям.
    for i, j in key:
        grid[i][j] = next(text_iter, random.choice(alphabet))

def extract_chars(grid, key):
    # [КРИПТО] Сборка букв из ячеек сетки, которые совпадают с текущими отверстиями.
...
# Добавляем случайные символы, если текст слишком короткий
    ...
    # [КРИПТО] Цикл из 4 поворотов решётки. На каждом шаге в открытые ячейки записывается часть текста.
...
def decrypt(cipher_text, key, size):
    ...
    # [КРИПТО] Цикл из 4 поворотов решётки. На каждом шаге собираются буквы из отверстий в том же порядке, что и при шифровании.
    for rotation in range(4):
        print(f"\n{'─' * 80}")
        print(f"Поворот {rotation + 1}/4 (угол {rotation * 90}°)")
        print(f"{'─' * 80}")
    ...
    result = restore_special_chars(result)# Восстанавливаем спецсимволы из словесных маркеров
    return result
...
def generate_valid_cardano_key(size):
    ...
    # [КРИПТО] Цикл подбора отверстий так, чтобы при 4 поворотах они не пересекались и заполняли всю сетку ровно один раз.
