...
# [КРИПТО] Размер алфавита для модульной арифметики RSA-хеша
...
def prepare_text(txt: str) -> str:
    ...
    # [КРИПТО] Приведение сообщения к каноническому виду перед хешированием
...
def restore_text(txt: str) -> str:
    ...
    # [КРИПТО] Обратное преобразование маркеров для читаемого вывода
...
def hash_message(text: str, N: int) -> int:
    ...
    h = 0
    # [КРИПТО] Цикл вычисления хеша: суммирование индексов символов с последующим квадратичным рассеиванием
    for ch in text:
        if ch in ALPHABET:
            mi = ALPHABET.index(ch) + 1
            # [КРИПТО] Аккумуляция: h_{i+1} = (h_i + m_i) mod N
            h = (h + mi) % N
            # [КРИПТО] Квадратичное преобразование для усиления диффузии (простой эвристический хеш)
            h = (h * h) % N
    return h if h != 0 else 1

def is_prime(n: int) -> bool:
    """Проверка числа на простоту"""
    # [КРИПТО] Тест простоты для генерации безопасных p, q в схеме RSA
    if n < 2:
        return False
    if n == 2:
        return True
...
def extended_gcd(a: int, b: int):
    ...
    # [КРИПТО] Рекурсивный расчёт НОД и коэффициентов Безу (x, y) для нахождения модульного обратного
    if b == 0:
        return a, 1, 0
...
def mod_inverse(a: int, m: int) -> int:
    ...
    # [КРИПТО] Нахождение d такого, что (a * d) ≡ 1 (mod m). Критично для вычисления закрытой экспоненты RSA
...
def input_int(prompt: str, low: int = None, high: int = None, is_prime_needed: bool = False) -> int:
    ...
    # [КРИПТО] Контроль ввода параметров ключевой пары с валидацией простоты и диапазонов
    while True:
        try:
...
def rsa_sign(message_hash: int, d: int, n: int) -> int:
    ...
    # [КРИПТО] Операция подписания: возведение хеша в степень закрытого ключа d по модулю N
    return pow(message_hash, d, n)

def rsa_verify(message_hash: int, signature: int, e: int, n: int) -> bool:
    """Проверка подписи: (S^e mod N) == H"""
    # [КРИПТО] Операция верификации: восстановление хеша открытым ключом e и сравнение с оригиналом
...
def demo_signature():
    ...
    p_demo, q_demo = 23, 29
    # [КРИПТО] Вычисление модуля N = p * q, определяющего размер кольца вычетов
    n_demo = p_demo * q_demo
    # [КРИПТО] Функция Эйлера φ(N) = (p-1)(q-1), используемая для генерации ключевой пары
    phi_demo = (p_demo - 1) * (q_demo - 1)
    e_demo = 13
    # [КРИПТО] Вычисление закрытой экспоненты: d ≡ e^{-1} (mod φ(N))
    ...
    # [КРИПТО] Хеширование сообщения перед подписанием для обеспечения целостности
    h = hash_message(prepared, n_demo)
    print(f"Подготовленное сообщение: {prepared} ")
    print(f"Хеш сообщения h = {h} ")

    # [КРИПТО] Формирование цифровой подписи S = h^d mod N
    signature = rsa_sign(h, d_demo, n_demo)
    print(f"\nГенерация подписи: ")
    print(f"  S = h^d mod N = {signature} ")

    # [КРИПТО] Проверка корректности подписи открытым ключом
...
def sign_message():
    ...
    # [КРИПТО] Ввод простых чисел p, q. Их битовая длина определяет криптостойкость модуля N
    ...
    # [КРИПТО] Проверка взаимной простоты e и φ(N): необходимое условие существования d
    while gcd(e, phi) != 1:
...
def verify_signature():
    ...
    # [КРИПТО] Сравнение восстановленного хеша с оригинальным: подтверждение аутентичности и целостности
