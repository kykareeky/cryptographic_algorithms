...
def input_with_constraints(prompt: str, min_val: int = None, max_val: int = None, 
    ...
    """Безопасный ввод числа с отображением ограничений в подсказке"""
    # Формируем текст ограничений
    constraints = []
...
def demo_diffie_hellman():
    """Демонстрация работы протокола на тестовых параметрах"""
    print("\n" + "=" * 60)
    print("ДЕМОНСТРАЦИЯ ПРОТОКОЛА ДИФФИ-ХЕЛЛМАНА")
    print("=" * 60)
    
    # Тестовые параметры (малые для наглядности)
    n_demo = 23  # простое
    a_demo = 5   # генератор
    Ka_demo = 6  # секрет Алисы
    Kb_demo = 15 # секрет Боба
...
def diffie_hellman():
    """Основная функция обмена ключами"""
    print("\n" + "-" * 60)
    ...
    # Валидация открытого ключа Алисы
    if Ya == 1:
    ...
    # Валидация открытого ключа Боба
    if Yb == 1:
    ...
    # Проверка безопасности общего секрета
