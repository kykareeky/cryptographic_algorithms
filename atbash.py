...
def prepare_text(text):
    ...
    case_info = []
    cleaned_text = []
    # [КРИПТО] Цикл подготовки текста: сохраняет исходный регистр букв для последующего восстановления после замены
...
def atbash_cipher(text):
    ...
    alphabet = 'АБВГДЕЖЗИЙКЛМНОПРСТУФХЦЧШЩЪЫЬЭЮЯ'
    # [КРИПТО] Создание обратного алфавита: последняя буква становится первой, вторая с конца — второй и т.д.
    reversed_alphabet = alphabet[::-1]
    # [КРИПТО] Формирование таблицы замены: каждая буква алфавита сопоставляется с симметричной ей буквой
    atbash_dict = {alphabet[i]: reversed_alphabet[i] for i in range(len(alphabet))}
    result = []
    # [КРИПТО] Цикл побуквенной замены: если символ есть в таблице, он меняется на противоположный
    # Шифрование и расшифрование выполняются одинаково, так как операция замены симметрична
    for char in text:
...
def main():
    ...
    if choice == '1':
        ...
        while True:
            line = input()
            if line == "":  # ИСПРАВЛЕНО: пустая строка корректно завершает ввод
                break
            lines.append(line)
        original_text = "\n".join(lines)
