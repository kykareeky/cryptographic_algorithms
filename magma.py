...
class CryptoSystem:
    ...
    def magma_inverse_transform(self, input_hex):
        """Обратное преобразование для МАГМА (создание обратных S-блоков)"""
        input_hex = input_hex.lower()
        if len(input_hex) != 8 or not all(c in '0123456789abcdef' for c in input_hex):
            return "Ошибка: введите 8 hex символов"
    ...
    def preprocess_text(self, text):
        """Базовая предобработка текста (заглушка для совместимости)"""
        return text

    def convert_punctuation(self, text, to_word=True):
        """Замена пунктуации на слова-маркеры или обратно"""
        if to_word:
            replacements = {
                ',': 'зпт', '.': 'тчк', ' ': 'прб', '"': 'квч',
    ...
    def text_to_hex(self, text):
        """Преобразование текста в hex-строку"""
    ...
    def hex_to_text(self, hex_string):
        """Преобразование hex-строки обратно в текст"""
        try:
    ...
    def magma_encrypt_hex(self, input_hex):
        """Шифрование hex-строки с использованием преобразования МАГМА"""
    ...
    def magma_decrypt_hex(self, hex_text):
        """Расшифрование hex-строки с использованием обратного преобразования МАГМА"""
...
def get_hex_input():
    """Функция для получения hex-строки от пользователя"""
...
def main():
    ...
    # Создаем тестовые файлы без вывода сообщений
    ...
    while True:
        ...
        if cipher_choice == '1':
            ...
            # Попытка конвертации обратно в текст
            try:
